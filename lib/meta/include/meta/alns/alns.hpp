...
template <typename Solution>
auto optimize(Config<Solution> const &config, RandomEngine &gen, auto local_optimize) -> Solution {
  ...
  for (size_t i = 0; i < config.max_iterations; ++i) {
    ...
    if (config.acceptance_criterion->accept(new_cost, old_cost, best_cost, gen)) {
      ...
      if (new_cost < best_cost) {
        ...
        // TODO: fix slightly convoluted logic
        auto optimized = best_solution;
        local_optimize(optimized);
        auto optimized_cost = optimized.cost();
        if (optimized_cost < best_cost) {
          best_solution = std::move(optimized);
    ...
    if (config.verbose && i % 100 == 0) {
      std::println("Iteration: {}, Current cost: {}, Best const: {}", i, old_cost,
                   best_solution.cost());
      assert(old_cost >= best_solution.cost());
      config.operator_selection->print_weights();
    }
  }
